...
import { createProcess, ProcessManager } from "./process-manager";
...
function print(color: keyof typeof colors, message: string): void {
...
async function runProcess(): Promise<void> {
  ...
  try {
    ...
    print("red", `Failed to start process: ${(error as Error).message}`);
    rl.close();
    process.exit(1);
  }
}
