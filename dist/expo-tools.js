 ...
 */
export function addExpoTools(server) {
        ...
        execute: async (args, { log }) => {
            try {
                ...
                global.processes.expo = await createProcess({
                    ...
                    onExit: (code, signal) => {
                        log.info(`Expo process exited with code ${code} and signal ${signal || "none"}`);
                    },
                });
                log.info(`Expo server started with PID: ${global.processes.expo.getPid()}`);
                return {
                    content: [
                        {
                            type: "text",
    ...
    server.addTool({
        ...
        execute: async (args, { log }) => {
            try {
                ...
                if (logType !== "all") {
                    filteredLogs = global.expoLogs.filter((entry) => entry.type === logType);
                }
                // Get the last N logs
...
export function setupShutdownHandlers() {
    const handleShutdown = async () => {
        if (global.processes) {
            console.error("Shutting down MCP server, stopping all managed processes...");
            for (const processKey in global.processes) {
                if (global.processes[processKey] &&
...
//# sourceMappingURL=expo-tools.js.map